...
var Api = {
	...
	// 获取附件(返回本地路径名)，用于直接打开附件
	// added by xiaopan
	// callback : filePath
	getAttachLocalPath: function (noteId, fileId, callback) {
		var me = this;
		// chech if fileId is local or server
		Note.getNote(noteId, function (note) {
			if (!note) {
				return callback && callback(false);
			}

			// check the file id type
			// console.log('\n=====1=======\n' + noteId + '\n================\n');
			// console.log('\n=====2=======\n' + fileId + '\n================\n');
			var isLocal = true;
			var realFileId = null;
			...
			for (var i in attachs) {
				...
				if (attach.FileId == fileId) {
					theAttach = attach;
					// have this fielID, chech if have serverFileID
					if (attach.ServerFileId) {
						isLocal = false;
						realFileId = attach.ServerFileId;
					} else {
						isLocal = true;
						realFileId = fileId;
					}
					break;
				} else if (attach.ServerFileId == fileId) {
					theAttach = attach;
					isLocal = false;
					realFileId = fileId;
					break;
			...
			var filePath = null;
			if (isLocal) {
				var myPath = attach.Path;
				fs.stat(myPath, function (err, stat) {
					if (err == null) {
						// we have this file with local id
						filePath = myPath;
						// console.log('\n------3.1>>>>\n' + filePath);
						callback(filePath);
					}
				...
				fs.stat(myPath, function (err, stat) {
					if (err == null) {
						// we have this file with service id
						filePath = myPath;
						// console.log('\n------4.1>>>>\n' + filePath);
						callback(filePath);
					} else if (err.code == 'ENOENT') {
						// download from server
						var url = me.getUrl('file/getAttach', { fileId: realFileId });
						// console.log('\n------4.2>>>>\n' + url);
						needle.get(url, function (err, resp) {
							me.checkError(err, resp);
							if (!err) {
								...
								// temp filename
								var newFileName = realFileId + '.' + type;
								var attachPath = User.getCurUserAttachsPath();
								var attachPathAll = attachPath + '/' + newFileName;
								// console.log('\n------4.2.1>>>>\n' + attachPathAll);
								fs.stat(attachPathAll, function (err, stat) {
									if (err == null) {
										// we have this file with service id and stored
										filePath = attachPathAll;
										theAttach.Path = attachPathAll;
										// console.log('\n------4.2.2.1>>>>\n' + filePath); 
										callback(filePath);
									} else if (err.code == 'ENOENT') {
										// download it
										fs.writeFile(attachPathAll, resp.body, function (err) {
											if (!err) {
												// we have this file with service id and downloaded
												filePath = attachPathAll;
												theAttach.Path = attachPathAll;
												// console.log('\n------4.2.2.2>>>>\n' + filePath);
												callback(filePath);
								...
								console.log('Failed to get attachment file from server!');
								return callback && callback(false);
							}
						});
					}
